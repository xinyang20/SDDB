...
async function checkUsername() {
  ...
  try {
    const response = await fetch(
      `/admin/users/validate_username?username=${encodeURIComponent(username)}`
    );
...
function updateRoleForm() {
  ...
  if (role === "patient") {
    ...
    roleForm.innerHTML = `
            ...
            <input type="text" name="department" placeholder="科室" required>
            <label for="title">职称:</label>
            <input type="text" name="title" placeholder="职称" required>
            <label for="contact_number">联系方式:</label>
            <input type="text" name="contact_number" placeholder="联系方式" required>
        `;
  } else if (role === "worker") {
